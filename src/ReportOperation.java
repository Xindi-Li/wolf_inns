...
public class ReportOperation{
		...
		while (true) {	
			...
			switch (input) {
				case "1":
					...
					choice = sc.next();
					if (Pattern.matches(pattern, choice)){
						System.out.println(getNumberOfAvailableRoomByHotelID(choice));
						break;
					}
					else {
						System.out.println("Your input is illegal");
				...
				case "2":		
					...
					choice = sc.next();
					if (Pattern.matches(patternForCategory, choice)){
						System.out.println(getNumberOfAvailableRoomByCategory(getCategory(Integer.valueOf(choice))));
						break;
					}
					else {
						System.out.println("Your input is illegal");
				...
				case "3":		    
			    	...
			    	if (Pattern.matches(patternForDate, choice)){
			    		...
			    		choice = sc.next();
			    		if (Pattern.matches(patternForDate, choice)){
			    			System.out.println(getNumberOfAvailableRoomByPeriod(start_date, choice));
			    			break;
			    		}
			    		else{
			    			System.out.println("Your input is illegal.");
	...
	public static int getNumberOfAvailableRoomByHotelID(String id){
		String sql = "select count(*) as total from room where hotel_ID = ? and availability = 1";
		Connection conn = DBconnection.getConnection();
		try {
			// System.out.println("int try segment");
            PreparedStatement ptmt = conn.prepareStatement(sql);
	...
	public static int getNumberOfAvailableRoomByCategory(String str){
		String sql = "select count(*) as total from room where room_category = ? and availability = 1";
		Connection conn = DBconnection.getConnection();
		try {
			// System.out.println("int try segment");
            PreparedStatement ptmt = conn.prepareStatement(sql);
